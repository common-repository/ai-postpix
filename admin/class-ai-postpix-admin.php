...
class AIPSTX_Admin {
	...
	public function add_plugin_admin_menu() {
		add_menu_page(
			'AI Postpix', // Sayfa başlığı
			'AI Postpix', // Menü başlığı
			'manage_options', // Yetenek
			$this->plugin_name,
			array($this, 'aipstx_display_ai_image_gen_page'), // İşlev
			plugins_url('img/pxmini.png', dirname(__FILE__)), // İkon URL
			19 // Pozisyon
		);

		// "AI Image Generator" alt menü öğesi olarak ekleniyor
		add_submenu_page(
			$this->plugin_name,
			'AI Postpix > AI Image Generator', // Sayfa başlığı
			'AI Image Generator', // Menü başlığı
			'manage_options', // Yetenek
			$this->plugin_name,
			array($this, 'aipstx_display_ai_image_gen_page') // İşlev
		);

		// "Settings" alt menü öğesi olarak ekleniyor
		add_submenu_page(
			$this->plugin_name,
			'AI Postpix > Settings', // Sayfa başlığı
			'Settings', // Menü başlığı
			'manage_options', // Yetenek
			$this->plugin_name . '-settings', // Menü slug'ı
			array($this, 'aipstx_display_plugin_settings_page') // İşlev
		);
	}
	...
	function aipstx_custom_admin_dark_mode() {
		...
		// Check if on the admin.php page and the 'page' query starts with 'ai-postpix'
		if ($pagenow == 'admin.php' && isset($_GET['page']) && strpos(sanitize_text_field($_GET['page']), 'ai-postpix') === 0) {
	...
	public function aipstx_save_settings_callback() {
		...
		if (isset($_POST['postpix-settings-submit'])) {  // Form gönderme kontrolü
			...
			if (!isset($_POST['aipstx_settings_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['aipstx_settings_nonce'])), 'aipstx_settings_action') || !current_user_can('manage_options')) {
				...
				update_option('aipstx_stability_key', sanitize_text_field($_POST['aipstx_stability_key']));
				...
				$show_metabox = isset ($_POST['aipstx_show_metabox']) ? '1' : '0'; // Checkbox işaretli ise '1', değilse '0' olarak ayarla
				$show_metabox = sanitize_text_field($show_metabox); // Sanitize edin
				update_option('aipstx_show_metabox', $show_metabox);
	...
	function aipstx_test_edenai_ajax_handler() {
		...
		$plugin_name = AIPSTX_NAME;
		$version = AIPSTX_VERSION;
	...
	public function aipstx_display_plugin_settings_page() {
		include_once('partials/ai-postpix-admin-display.php');
	}

	public function aipstx_display_ai_image_gen_page() {
		$img_gen_page = new \AIPSTX\AIPSTX_admin_genimg();
		$img_gen_page->aipstx_display();
	}

	public function options_update() {
		register_setting('postpix', 'aipstx_edenai_key');
		register_setting('postpix', 'aipstx_openai_key');
		register_setting('postpix', 'aipstx_stability_key');
		register_setting('postpix', 'aipstx_prompt_engine');
		register_setting('postpix', 'aipstx_theme_style');
		register_setting('postpix', 'aipstx_show_metabox');
	}
